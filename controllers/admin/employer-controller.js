...
export const createEmployer = (req, res) => {
  const val = new Validator(req.body, {
    ...
    companyLogoSecureUrl: 'required',
    companyLogoPublicId: 'required',
    ...
    .then((matched) => {
      if (!matched) {
        ...
        Employer.create(request).then((employer) => {
          ...
          VerifyEmail.create(verificationData).then((verificationBody) => {
            ...
            return ejs.renderFile(
              path.join(
                __dirname.replace('server', 'templates'),
                'emails',
                'account-verification.ejs'
              ),
              {
                link: verificationBody.link,
                name: employer.name,
                model: 'employer'
              },
              (error, body) => {
...
export const getEmployerProfile = (req, res) => {
  return Employer.findOne({ id: req.params.id }, { password: 0, __v: 0 })
    .exec()
    .then((profile) => {
...
export const updateProfile = (req, res) => {
  ...
  if (companyLogoPublicId) {
    data.companyLogoPublicId = companyLogoPublicId
  }
  if (companyLogoSecureUrl) {
    data.companyLogoSecureUrl = companyLogoSecureUrl
  }
